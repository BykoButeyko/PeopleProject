...
contract("People", async function(accounts){
    it("Should set senior status properly", async ()=>{
        ...
        await instance.createPerson("Barbara", 65, 171, {value: web3.utils.toWei("1", "ether"), from: accounts[0]});
    ...
    it("Sholudn't be deleted by an another account owner", async ()=>{
        let instance = await People.deployed();
        await truffleAssert.fails(instance.deletePerson(accounts[0], {from: accounts[5]}),
        truffleAssert.ErrorType.REVERT);
    });
    it("Should be able to delete your own account", async ()=>{
        let instance = await People.deployed();
        let person = await instance.getPerson();
        await truffleAssert.passes(instance.deletePerson(accounts[5], {from: accounts[0]}))
